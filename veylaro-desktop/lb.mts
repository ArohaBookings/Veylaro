...
while (!done) {
    ...
    for await (const part of veylaroChat(URL, "", convo, TIER, false, undefined, {})) {
      ...
      for (const ev of parser.push(part.chunk)) {
        if (ev.t === "file") {
          ...
          deliverable.set(ev.path, joined);
          wrote++;
          console.log(`  appended ${ev.path} (+${ev.content.split("\n").length} lines)`);
  ...
  if (isProtocolFailure(wrote, 0)) {
    convo.push({ role: "user", content: enforcementBrief({ request: TASK, missing: verdict.missing,
      existingPaths: [...deliverable.keys()], attempt: consecutiveIdle }) });
    continue;
  }
  const reg = detectRegression(beforeStep, deliverable);
  if (reg.regressed) {
    console.log(`  -> REGRESSION (${reg.linesBefore} -> ${reg.linesAfter})`);
    convo.push({ role: "user", content: regressionBrief(reg) });
    continue;
  }
  if (!verdict.complete) {
    const more = /across \d+\+ files|several real screens|product surface/i.test(verdict.missing.join(" "));
    const b = more ? breadthBrief(TASK, [...deliverable.keys()], ambitionFloor(TASK).files) : null;
    convo.push({ role: "user", content: b ?? continuationBrief(verdict) });
    continue;
  }
  convo.push({ role: "user", content: continuationPressure({ step, consecutiveIdle, deliverableComplete: true, requestedDone, aborted: false }, plan) });
}
